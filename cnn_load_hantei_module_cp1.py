...
def cnn(pix,fil,image):
    ...
    NUM_CLASSES = 2
    IMAGE_SIZE = pix
    IMAGE_4_SIZE = int(IMAGE_SIZE/2) #2^cpで除算
    ...
    def inference(images_placeholder, keep_prob):
        ...
        with tf.name_scope('fc1') as scope:
            W_fc1 = weight_variable([IMAGE_4_SIZE*IMAGE_4_SIZE*c1_filter, 512])
            b_fc1 = bias_variable([512])
            h_pool1_flat = tf.reshape(h_pool1, [-1, IMAGE_4_SIZE*IMAGE_4_SIZE*c1_filter])
    ...
    with tf.Graph().as_default():
        ...
        # モデルを復元
        saver.restore(sess, "./modelcp1/model.ckpt")
